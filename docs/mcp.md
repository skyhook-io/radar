...
| `get_helm_release` | Detailed Helm release info with owned resources, resource health, Flux ownership, and current `lastOperation`; use `include=history,operations` for the full Helm revision trail and `include=diff` for manifest comparison. For releases with `storageNamespace`, pass that value as `namespace`. | `namespace` (required: Helm storage namespace), `name` (required), `include` (optional: `values,history,operations,diff`), `diff_revision_1` (required when `include=diff`) / `diff_revision_2` (optional) |
