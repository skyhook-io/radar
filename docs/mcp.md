...
| `get_resource` | Detailed view of a single resource — minified spec + status + metadata + default-on `resourceContext` (managedBy / exposes / selectedBy / uses / runsOn / scheduling / issue+audit rollups). Exact `kueue.x-k8s.io/v1beta2` Workloads get one bounded Kueue admission observation under `scheduling.observations`: a controller-neutral `satisfied` / `unsatisfied` / `held` / `unknown` decision; subject generation paired with the native primary condition's observed generation; role-bearing LocalQueue and ClusterQueue facts; AdmissionCheck and preemption gates; current disruption conditions; and typed Kueue phase, PodsReady/replacement-Pod evidence, PodSet flavor/usage assignments, requeue, and concurrent-admission Parent data. `held` is reserved for proven explicit pauses, including an `AdmissionGated` condition paired with the current `admission-gated-by` annotation; ambiguous inactive-queue and preemption blocks remain `unsatisfied`. Provider names remain visible when RBAC withholds optional navigation refs. The compact assignment projection prioritizes extended resources and reports truncation; the returned Workload remains the complete source. Admission is not Pod placement or physical capacity, and normal unsatisfied admission is not automatically an Issue. Other Kueue API versions do not receive this adapter. `issueSummary` uses live-operational `critical`/`warning`; `auditSummary` uses the Checks posture-remediation ladder `critical`/`high`/`medium`/`low` (current built-ins are `high`/`medium`) and is not evidence of an active outage. Optionally include heavier supplemental data (events / metrics). For logs use `get_pod_logs` / `get_workload_logs` / `diagnose`. | `kind` (required), `namespace` (optional — omit for cluster-scoped kinds: Node, ClusterRole, IngressClass, etc.), `name` (required), `group` (optional, for ambiguous kinds), `include` (optional: `events,metrics,changes,revisions` — `revisions` lists rollback targets for Deployment/StatefulSet/DaemonSet/Rollout with `number`, `image`, `createdAt`, `isCurrent`; Rollouts also mark `isStable`, the revision an abort reverts to), `context` (optional: `basic` default, `none` for bare minified output) |
