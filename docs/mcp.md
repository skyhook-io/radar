...
| `get_events` | Recent Kubernetes events, deduplicated and sorted by recency — Warning-only by default. Filter by resource kind/name to scope; `type=Normal`/`all` includes lifecycle events as timeline evidence. | `namespace` (optional), `limit` (optional, default 20, max 100), `kind` (optional), `name` (optional), `type` (optional: `Warning` default, `Normal`, `all`) |
