...
| `get_helm_release` | Detailed Helm release info with current `lastOperation` when Helm history indicates a failed upgrade, rollback-after-failure, rollback, or stuck pending operation; optional values, history, operations, and manifest diff | `namespace` (required), `name` (required), `include` (optional: `values,history,operations,diff`), `diff_revision_1` (required when `include=diff`) / `diff_revision_2` (optional) |
