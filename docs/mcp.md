...
### Instrumentation and Write Tools

| Tool | Description | Parameters |
|------|-------------|------------|
| `inspect_pod_runtime` | Collect short-lived kernel evidence for a Pod container using an existing Inspektor Gadget deployment. This temporarily loads a pinned eBPF gadget on the target node and unloads it after the bounded capture; it does not install Gadget or mutate Kubernetes resources. Process snapshots expose command names, not argv. DNS output is aggregated and raw packets are not returned. | `namespace`, `pod`, `aspect` (`processes`, `connections`, `dns`); optional `container` (first running app container by default); `duration` for DNS (default 10s, max 30s) |
