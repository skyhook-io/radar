...
For `issues`, `first_seen` is the earliest evidence-backed time the issue was active. It may be a Kubernetes transition/event timestamp or the time Radar first observed the state, so read it as “active at least since,” not exact onset. A missing value is exposed to CEL as `0`; every logical branch that compares `first_seen` as an age must guard known timing with `first_seen != 0`, `!onset_unknown`, or `onset_coverage_unknown == 0`, otherwise Radar rejects the filter instead of silently treating unknown onset as 1970. `onset_coverage` is emitted only when at least one contributing signal lacks an active-time anchor. `resource_created_at` is resource-age context and the stable ordering fallback, never onset evidence. `issue_timing` is independent evidence. In particular, `issue_timing_basis: owner_condition` is workload-level health evidence, not timing or attribution for the row's specific reason/cause, and need not share the `first_seen` anchor. Do not subtract `resource_created_at` from `first_seen` to infer a healthy duration.
