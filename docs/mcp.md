...
**The story contract.** The reply prose (`report`) is the assessment's story, and it may place a Radar result with `[[radar:evidence=N]]` alone on a line — N is the 0-based index into `evidence` — or reference a placed result with the same marker inline. Findings renders the placed result's card at that position from Radar's own record; the agent never authors a card. A marker inside a code span, a code block or a blockquote is literal text. At most 6 results are placed; later placements become references; a marker Radar cannot resolve renders a visible "support unavailable" note at the sentence rather than vanishing. The verdict adds `summary` (1–2 plain sentences, ≤ 280 chars), `certainty` (`established` | `likely` | `suspected`, rendered as the agent's word, never a Radar mark), `unresolved` (≤ 3 items, shown above the story fold whenever present — and their absence is shown too), typed `steps` (`text`, `kind` of `mitigate` | `verify` | `investigate`, optional `precondition`; when present the server derives `remediation` from them so one list feeds Apply and `recommended_index`, which may only name a `mitigate` step), and on follow-up turns `revises_assessment`, which the server keeps only on a complete verdict (`summary` plus a root cause, healthy or inconclusive). Findings shows the newest assessment turn — initial, verification, or a question whose verdict revised it — and nothing else; Activity keeps every earlier assessment complete. With typed steps present, `inconclusive` is cleared only by a non-empty `root_cause` (a mitigation never establishes a cause). No role can hide, collapse, or recolor a card, and a claim that cannot be pinned to exactly one observation shows beside its source in Assessment details instead. A backend that sends no `summary` and no placement markers renders in the previous shape.
