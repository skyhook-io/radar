...
For `issues`, `first_seen` is the earliest evidence-backed time the issue was active. It may be a Kubernetes transition/event timestamp or the time Radar first observed the state, so read it as “active at least since,” not exact onset. A missing value is exposed to CEL as `0`; age filters should guard known timing with `first_seen != 0`, `!onset_unknown`, or `onset_coverage_unknown == 0` so unknown onset is not treated as 1970. `onset_coverage` is emitted only when at least one contributing signal lacks an active-time anchor. `resource_created_at` is resource-age context and the stable ordering fallback, never onset evidence. `issue_timing` is independent evidence. The `owner_condition`, `pod_creation`, and `spec` bases classify workload-level or structural timing without dating or attributing the row's specific reason/cause, so they need not share the `first_seen` anchor. Do not subtract `resource_created_at` from `first_seen` to infer a healthy duration.
