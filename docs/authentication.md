...
- Adds a separate **extended API read add-on** for each enabled Cloud tier, granting exact `get/list/watch` access to the namespaced Kubernetes 1.37 Workload, PodGroup, CompositePodGroup, and PodCertificateRequest APIs. The scheduling APIs are feature-gated; PodCertificateRequest is stable. ClusterTrustBundle stays in the cluster-read add-on so disabling cluster-scoped access still has the expected effect. Neither role grants CertificateSigningRequest, signer approval, Secrets, or write verbs.
