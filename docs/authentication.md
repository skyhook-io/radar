...
- Binds `radar:system` — the group the hub's alerts worker and timeline puller assert — to a read-only set: `view`, the cluster-read and integration-read add-ons, and Secret read (Helm stores releases as Secrets, and Helm release alerts list them as the caller). Controlled by `cloud.systemRbac` (default on) and independent of `cloud.defaultRbac`, so orgs that turn the role bindings off to let IdP groups (`radar:idp:<id>`) decide cluster access keep alerts and the hub timeline.
- Uses the Cloud role (`radar:owner` / `radar:member` / `radar:viewer`) only to gate Radar's own configuration (settings, integrations, audit settings, self-upgrade), which runs as Radar's ServiceAccount. Operations that run as the impersonated user — Helm reads and writes, in-cluster probe pods — are decided by Kubernetes RBAC alone.
