# Cloud default integration-read policy

Radar's connector collects resources with its service account. A Cloud user's
...
group/resource identities. Each enabled Cloud tier gets its own bindings to
...
workflow parameters, database connection configuration and Traefik plugin settings
...
| CAPI KubeadmConfig, KubeadmConfigTemplate, KubeadmControlPlane, KubeadmControlPlaneTemplate | Bootstrap configuration can contain inline files and credentials; the renderer-only control-plane template is also withheld pending credential-field review. Other curated machine/cluster inventory remains readable. |
...
`cloud.defaultRbac.create=false`. Cloud can still forward IdP groups as
...
Upgrading only the binary or using Cloud self-upgrade does not install these RBAC
