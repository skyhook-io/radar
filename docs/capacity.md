...
- A claim deleted before ever reaching Ready — the registration-timeout path — terminalizes its provision episode as **failed**, so "nodes not joining" never reads as provisioning still in progress. The deletion signal itself carries no cause, so a claim removed for other reasons (manual deletion, a NodePool being deleted with claims still launching) reads the same way; distinguishing those deletions is a known refinement.
