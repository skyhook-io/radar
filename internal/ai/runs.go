...
// activeTurnLocked returns the durable marker of the currently reserved turn.
// Caller holds r.mu.
func (r *Run) activeTurnLocked() StreamEvent {
	if !r.inFlight {
		return StreamEvent{}
	}
	for i := len(r.events) - 1; i >= 0; i-- {
		if r.events[i].Event.Type == "turn" {
			return r.events[i].Event
		}
	}
	return StreamEvent{}
}
