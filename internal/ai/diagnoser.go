...
// machine against Radar's HTTP MCP endpoint (no tunnel; optional local session
// token on write-capable /mcp), and Radar never calls an LLM itself — the model
// auth lives entirely in the user's CLI.
//
...
type Request struct {
	...
	// Read-only investigation and /mcp-readonly turns do not send it.
	MCPToken string
