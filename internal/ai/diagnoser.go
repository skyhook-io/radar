...
const verdictContract = "VERDICT BLOCK: " +
	...
	"root_cause: the paragraph under the headline, two or three plain sentences for a reader who will not open the analysis: what failed, the mechanism, and the one result that shows it, with since-when if a result gives it. GitHub-flavored markdown; backticks only for an identifier the reader needs exactly, not for every name. Claim nothing the ledger does not cover; empty when healthy or inconclusive. " +
