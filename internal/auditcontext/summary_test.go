...
func TestSummarizeFindingsUsesNativePostureSeverity(t *testing.T) {
	...
	if summary.HighestSeverity != bpaudit.SeverityDanger {
		t.Errorf("HighestSeverity = %q, want native audit severity %q", summary.HighestSeverity, bpaudit.SeverityDanger)
	}
...
func TestSummarizeFindingsIsGroupAwareAndDeterministic(t *testing.T) {
	...
	if summary.Count != 2 || summary.HighestSeverity != bpaudit.SeverityWarning || summary.TopFinding != "a-check" {
		t.Fatalf("summary = %#v, want two warning findings with a-check first", summary)
	}
...
func TestSummarizeFindingsOmitsEmptyAndPassesThroughFutureSeverity(t *testing.T) {
	...
	findings := []bpaudit.Finding{{
		...
		CheckID: "future-check", Severity: "future",
	}}
	summary, _ := summarizeFindings(findings, "", "Pod", "prod", "api")
	if summary == nil || summary.HighestSeverity != "future" {
		t.Fatalf("future severity summary = %#v, want passthrough", summary)
	}
}
