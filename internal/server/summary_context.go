...
func (s *Server) newSummaryContextBuilder(namespaces []string, kindFilter string) summaryContextBuilder {
	...
	return s.summaryContextBuilderFromIndexes(idx, idx)
}
...
// suffices.
func (s *Server) newSearchSummaryContextBuilder(scanNamespaces []string) summaryContextBuilder {
	provider := issues.NewCacheProvider()
	if provider == nil {
		return nil
	}
	...
	return s.summaryContextBuilderFromIndexes(namespacedIdx, clusterIdx)
}

// summaryContextBuilderFromIndexes is the shared closure body for the
// list and search variants. namespacedIdx is used for namespaced hits;
// clusterIdx is used for cluster-scoped hits. The list path passes the
// same index for both (single-kind list, scope already chosen by the
// caller); search passes two distinct indexes.
func (s *Server) summaryContextBuilderFromIndexes(namespacedIdx, clusterIdx issueIndex) summaryContextBuilder {
	...
	return func(obj runtime.Object, u *unstructured.Unstructured, group, kind, namespace, name string) *resourcecontext.SummaryContext {
		...
		// Dispatch by scope: cluster-scoped hits read from clusterIdx
		// (composed at namespace=nil so namespace="" issues are present),
		// namespaced hits read from namespacedIdx (which honors the
		// user's namespace filter so the per-row count doesn't pull in
		// noise from namespaces the user can't see).
		idx := namespacedIdx
