...
func TestWorkloadRatioPreservesGapsAndMeasuredZero(t *testing.T) {
	denominator := workloadMetricPanel{State: "available", Series: []prom.Series{{DataPoints: []prom.DataPoint{{Timestamp: 1, Value: 2}, {Timestamp: 2, Value: 0}, {Timestamp: 3, Value: 5}}}}}
	numerator := workloadMetricPanel{State: "available", Unit: "percent", Series: []prom.Series{{DataPoints: []prom.DataPoint{{Timestamp: 1, Value: 0.2}}}}}
	got := workloadRatioPanel(numerator, denominator, 100, true)
	points := got.Series[0].DataPoints
	if got.State != "available" || points[0].Value != 10 || !math.IsNaN(points[1].Value) || points[2].Value != 0 {
		t.Fatalf("incorrect errors: %+v", got)
	}
	coverage := workloadRatioPanel(numerator, denominator, 1, false)
	if !math.IsNaN(coverage.Series[0].DataPoints[2].Value) {
		t.Fatal("missing coverage invented")
	}
	if denominator.Series[0].DataPoints[0].Value != 2 {
		t.Fatal("mutated request rate")
	}
	numerator.State = "stale"
	if got := workloadRatioPanel(numerator, denominator, 100, true); got.State != "available" {
		t.Fatal("fresh zero-filled errors labeled stale")
	}
	if got := workloadRatioPanel(numerator, denominator, 1, false); got.State != "stale" {
		t.Fatal("historical coverage not labeled stale")
	}
	if got := workloadRatioPanel(workloadMetricPanel{State: "error"}, denominator, 100, true); got.State != "error" || len(got.Series) != 0 {
		t.Fatal("failure turned into zero")
	}
}

func TestWorkloadQueryBoundAndPartialResponse(t *testing.T) {
	called := false
	client := workloadQueryFunc(func(context.Context, string, time.Time, time.Time, time.Duration) (*prom.QueryResult, error) {
		called = true
		return nil, prom.ErrPartialResponse
	})
	now := time.Now()
	large := queryWorkloadPanel(context.Background(), client, strings.Repeat("x", 16001), nil, "cores", now, now, time.Minute, true)
	if called || large.State != "error" || !strings.Contains(large.Reason, "query limit") {
		t.Fatalf("query not bounded: %+v", large)
	}
	partial := queryWorkloadPanel(context.Background(), client, "up", nil, "cores", now, now, time.Minute, true)
	if !called || partial.State != "error" || len(partial.Series) != 0 || !strings.Contains(partial.Reason, "partial response") {
		t.Fatalf("partial response accepted: %+v", partial)
	}
}
...
func TestWorkloadCoverageWithholdsMismatchedSamples(t *testing.T) {
	...
	got = withMetricCoverage(panel, workloadMetricPanel{State: "error", Reason: "Query exceeds safe query limit."}, "partial histograms")
	if got.State != "error" || len(got.Series) != 0 || !strings.Contains(got.Reason, "query limit") {
...
func TestWorkloadWithoutConfigurationRequiresReadyCache(t *testing.T) {
	...
	if w.Code != http.StatusServiceUnavailable || !strings.Contains(w.Body.String(), "cluster cache not ready") {
		t.Fatalf("unexpected cache response: %d %s", w.Code, w.Body.String())
	}
}
...
func TestWorkloadCollectionKeepsObserversSeparate(t *testing.T) {
	...
	client := workloadQueryFunc(func(_ context.Context, query string, _, _ time.Time, _ time.Duration) (*prom.QueryResult, error) {
		...
		if strings.Contains(query, "count(count by (job,") || strings.HasPrefix(query, "min(count by (le)") {
			value = 1
		}
...
func TestWorkloadScopeDoesNotFollowConnectionChanges(t *testing.T) {
	for _, change := range []struct {
		...
		{"configure URL", func() { Configure("http://different:9090", nil) }},
		{"configure headers", func() { Configure("", map[string]string{"X-Scope-OrgID": "other"}) }},
...
func TestWorkloadScopeSurvivesUnchangedStartupAndSettings(t *testing.T) {
	...
	Configure("http://metrics:9090/", map[string]string{"X-Scope-OrgID": "tenant-a"})
	if _, _, ok := GetClient().workloadMetricsConfig(); !ok {
		t.Fatal("unchanged atomic configuration discarded scope")
	}
