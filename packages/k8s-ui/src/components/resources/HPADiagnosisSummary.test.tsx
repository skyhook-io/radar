...
import { HPADiagnosisSummary, hpaBadgeSeverity, hpaReasonGroups, isHPAReasonRedundant } from './HPADiagnosisSummary'
...
const maxed: HPADiagnosisView = {
  ...
  reasons: [
    ...
    { id: 'missing_current_metric', message: 'HPA is missing current metric values', detail: 'cpu' },
  ],
}
...
describe('hpaBadgeSeverity', () => {
  ...
  it.each(Object.entries(expected))('maps %s to the %s badge', (state, severity) => {
...
describe('isHPAReasonRedundant', () => {
  ...
  it('splits reasons into restatements and additions', () => {
    ...
    expect(groups.redundant.map((reason) => reason.id)).toEqual(['limited_max'])
    expect(groups.additional.map((reason) => reason.id)).toEqual(['missing_current_metric'])
  })
})

describe('HPADiagnosisSummary', () => {
  it('shows the detail variant evidence without repeating the headline', () => {
    const html = render(<HPADiagnosisSummary diagnosis={maxed} variant="detail" />)
  ...
  it('attributes the controller condition in the inline variant and lists the rest', () => {
    const html = render(
      <HPADiagnosisSummary
        diagnosis={maxed}
        variant="inline"
        header={<span data-testid="host-header">HorizontalPodAutoscaler api-hpa</span>}
      />,
    )
  ...
  it('omits the bounds line when the producer withheld bounds', () => {
    const html = render(
      <HPADiagnosisSummary diagnosis={{ state: 'ok', summary: 'HPA is tracking its target' }} variant="inline" />,
    )
