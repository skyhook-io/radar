...
import { togglePick, pickIndex, COMPARE_PICK_CAP, type Pick } from './picks'

const ref = (namespace: string, name: string): Pick => ({ namespace, name })
...
describe('togglePick', () => {
  ...
  it('removes existing pick (deselect)', () => {
    const start: Pick[] = [ref('prod', 'api'), ref('staging', 'api')]
