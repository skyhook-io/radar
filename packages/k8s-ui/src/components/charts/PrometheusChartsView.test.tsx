...
function result(patch: Partial<PrometheusResourceMetricsResult>): PrometheusResourceMetricsResult {
...
describe('describePodCoverage', () => {
  it('names the basis a chart was drawn on', () => {
    expect(describePodCoverage(result({ coverage: 'ksm_history', observedPods: 6 }))).toBe(
      '6 pods attributed to this workload in the window (kube-state-metrics)',
    )
    expect(describePodCoverage(result({ coverage: 'ksm_history', observedPods: 1 }))).toBe(
  ...
  it('says what the current-pod fallback cannot see', () => {
    ...
    expect(describePodCoverage(result({ coverage: 'current_pods', pods: 3, podsTotal: 3 }))).toBe(
      '3 current pods; pods replaced during the window are not included',
    )
    expect(describePodCoverage(result({ coverage: 'current_pods', pods: 50, podsTotal: 120 }))).toBe(
  ...
  it('carries the reason ownership history was unavailable', () => {
    expect(
      describePodCoverage(result({ coverage: 'current_pods', pods: 2, podsTotal: 2, scopeError: 'probe timed out' })),
    ).toBe(
