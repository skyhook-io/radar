...
- GitOps nodes: Application (ArgoCD), Kustomization, HelmRelease, GitRepository (FluxCD)
  ...
  - **Per-resource health provenance**: a tree node's `health` carries `healthSource` (`controller` = the GitOps CR's own verdict; `radar` = Radar's read: topology status, or the issues engine's finding overlaid by `overlayRadarHealth` in `internal/server/gitops_handlers.go`). Argo CD 3 doesn't persist per-resource health in the CR (`status.resourceHealthSource: appTree` → `ResourceTree.HealthMode`), so the overlay is what populates it there — problems only, never `Healthy`, in-cluster destinations only. Every consumer (node tone, Changes category, per-resource Issue, `Summarize`) reads the tree's resolved value; never re-read `status.resources[].health` elsewhere. The UI marks `radar` values (`radarHealthNote`) and `GitOpsHealthSourceNotice` explains once per page. See [docs/gitops.md](docs/gitops.md#per-resource-health)
