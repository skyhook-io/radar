...
- Workloads: `/api/workloads/{kind}/{ns}/{name}/...` (logs, restart, scale, revisions, rollback, images, history) — `history` is the workload view's timeline: the workload under its key (every incarnation), what it owns walked by owner UID, K8s Events about those, and resources attached now (Services, config, scalers, PDBs, NetworkPolicies, PVCs), never sibling workloads; newest arrival first with `truncated` + `nextBeforeSeq` → `before_seq` paging. `revisions`/`rollback` accept Deployment, StatefulSet, DaemonSet, and **Rollout**; `rollbackableWorkloadKinds` in `server.go` is the gate. `images` GET/POST accepts the same four kinds, uses compare-and-swap JSON Patch, and follows a Rollout's `workloadRef` to the referenced workload
