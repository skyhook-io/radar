...
- Nodes: `/api/nodes/{name}/...` (cordon, uncordon, drain (waits for the evicted pods to actually be deleted by default — kubectl parity — reporting still-terminating pods in `pendingPods`; `waitForDeletion:false` returns once evictions are accepted), drain-plan (read-only estimate: per-pod evict/skip/may-block with reasons), debug)
