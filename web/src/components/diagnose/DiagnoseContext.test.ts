...
describe("investigation workspace routes", () => {
  ...
  it("keeps only cross-view scope in workspace URLs", () => {
    expect(investigationWorkspaceSearch("?org=o1&namespaces=prod%2Cstage&ai-run=r1&resource=pod"))
      .toBe("?org=o1&namespaces=prod%2Cstage");
  });

  it("builds opaque navigation state for a synthetic drawer return", () => {
    expect(investigationWorkspaceNavigationState("r1", {
      returnPath: "/?ai-run=r1",
      drawerOrigin: true,
      closeHistorySteps: 2,
    })).toEqual({
      preferredInvestigationRunID: "r1",
  ...
  it("does not claim unrelated paths and rejects malformed run ids", () => {
    ...
    expect(isInvestigationWorkspacePath("/investigations/run/extra")).toBe(true);
