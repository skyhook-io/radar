...
import {
  ...
  AppInfoSection,
} from '../resources/drawer-components'
...
export function ResourceRendererDispatch({
  ...
  const sidebarContent = showCommonSections && (
    <>
      ...
      {kind !== 'events' && <EventsSection events={events || []} isLoading={eventsLoading} hint={eventsHint} />}
      ...
      <div className={clsx('p-4 space-y-4', renderSidebar && 'lg:flex-1 lg:min-w-0')}>
        ...
        {showCommonSections && (
          <>
            ...
            {showMetrics && !(kind === 'pods' && data?.status?.phase === 'Pending') && (
