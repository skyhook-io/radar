...
import { useKueueAdmission } from '../../api/client'
...
export function JobSetAdmission({ resource, namespace, name, onNavigate }: {
  ...
  return <KueueAdmissionSection data={query.data} loading={query.isLoading} error={query.error?.message} hinted={hinted} externalExecution={externalExecution} onRetry={() => { void query.refetch() }} onNavigate={onNavigate} />
}
