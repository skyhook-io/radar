...
function namespaceFilterDisabled(
  ...
  if (view === "cost" && !pathname.startsWith("/cost/rightsizing")) {
    return {
      disabled: true,
      tooltip:
        "Cost is reported per namespace across the whole cluster — the namespace filter doesn’t apply here.",
    };
  }
  if (view === "capacity") {
    return {
      disabled: true,
      tooltip:
        "Capacity is reported across the cluster — the namespace filter doesn’t apply here.",
    };
  }
  ...
  if (view === "gitops" && segments[1] === "detail") {
    return {
      disabled: true,
      tooltip:
        "This resource manages workloads across namespaces — the namespace filter doesn’t apply to its tree.",
    };
  }
  if (view === "resources") {
    const kindSlug = segments[1];
    const group = new URLSearchParams(search).get("apiGroup") || "";
    const match = kindSlug
      ? findAPIResourceForRoute(apiResources, kindSlug, group)
      : undefined;
...
function radarPageTitle(
  ...
  const pluralKindTitle = (kind: string, resourceName: string) =>
    kind.toLowerCase() === resourceName.toLowerCase() || /Metrics$/.test(kind)
      ? kind
      : englishPlural(kind);
  ...
  if (view === "workload")
    return pathSegments.slice(3).map(decode).join("/") || null;
  ...
  if (view === "resources") {
    ...
    return pluralKindTitle(
      match?.kind ?? pluralToKind(resourceName),
      resourceName,
    );
  }
  ...
  if (view === "gitops")
    return pathSegments[1] === "detail"
      ? decode(pathSegments[4] ?? "") || "GitOps"
      : "GitOps";
  if (view === "applications") {
    ...
    return slash >= 0 && slash < decoded.length - 1
      ? decoded.slice(slash + 1)
      : decoded;
  }
  if (view === "capacity") {
    if (pathSegments[1] === "pools")
      return decode(pathSegments[2] ?? "") || "Capacity";
...
function AppInner({
  ...
  const takeover: Record<FleetTakeoverTarget, string | undefined> = useMemo(
    () => ({
      ...
      checks:
        navCustomization.fleetTakeoverHref?.("checks") ??
        navCustomization.clusterChecksHref?.(),
  ...
  const namespaceFilter = namespaceFilterDisabled(
    mainView,
    location.pathname,
    location.search,
    navApiResources,
  ...
  useDocumentTitle(
    manageDocumentTitle
      ? radarPageTitle(location.pathname, location.search, navApiResources)
      : null,
    documentTitleSuffix,
  );
  ...
  const isViewTakenOver = (view: ExtendedMainView): boolean =>
    (view === "issues" || view === "gitops" || view === "checks") &&
    !!takeover[view];
  ...
  const peekRouteOrphaned =
    !!selectedResource &&
    !drawerExpanded &&
    mainView !== "resources" &&
    peekOwnerKeyRef.current !== null &&
    peekOwnerKeyRef.current !==
      peekOwnerKey(location.pathname, location.search);
  ...
  const appsInlineWorkloadActive =
    mainView === "applications" && searchParams.has("workload");

  const routeSelectedResource =
    (resourcesKindRouteChanged && selectedResourceRouteMismatch) ||
    peekRouteOrphaned ||
    appsInlineWorkloadActive
      ? null
      : selectedResource;
  ...
  const expandedView =
    !!routeSelectedResource && (searchParams.get("full") === "1" || isMobile);
  ...
  const drawerExpandedProp = routeSelectedResource
    ? expandedView
  ...
  const navigateToResource = useCallback(
    (res: SelectedResource, tab: "detail" | "yaml" = "detail") => {
      ...
      peekOwnerKeyRef.current = peekOwnerKey(
  ...
  const navigateToHelmRelease = useCallback(
    (namespace: string, name: string, storageNamespace?: string) => {
      const newParams = new URLSearchParams();
      const globalNamespaces = searchParams.get("namespaces");
      if (globalNamespaces) {
        newParams.set("namespaces", globalNamespaces);
  ...
  const { clusterLoadState, showHomeClusterLoadFallback, clusterLoadInitial } =
    useClusterLoadState({
      namespaces,
      mainView,
      chromeless,
  ...
  const showClusterWarmupLabel =
    clusterLoadState.loading && !(clusterLoadInitial && mainView === "home");
  ...
  const timelineInvalidationRef = useRef<{ timer: number | null }>({
    timer: null,
  });
  ...
  useEffect(
    () => () => {
      ...
      if (timelineInvalidationRef.current.timer !== null)
        clearTimeout(timelineInvalidationRef.current.timer);
      fastInvalidationRef.current = {
        changedKinds: new Set(),
        structuralKinds: new Set(),
        secretsChanged: false,
  ...
  const {
    ...
    connecting: eventStreamConnecting,
    reconnect: reconnectEventStream,
  } = useEventSource(
    namespaces,
  ...
  const liveUpdatesDisconnected =
    clusterConnected && !eventStreamConnected && !eventStreamConnecting;
  ...
  const headerReconnect =
    clusterConnectionState === "disconnected"
      ? retryConnection
      : reconnectEventStream;
  const headerReconnectPending =
    clusterConnectionState === "disconnected" ? isRetrying : false;
  ...
  useEffect(() => {
    ...
    if (!initialBookmarkReconciledRef.current) {
      ...
      if (!sameAsState && sortedState.length > 0) {
        if (
          namespaceScope.cacheScoped &&
          (!namespaceScope.namespaceRescope || sortedState.length !== 1)
        ) {
  ...
  useEffect(() => {
    ...
    if (urlNamespaces.join(",") !== namespacesKey) {
      if (
        namespaceScope?.cacheScoped &&
        (!namespaceScope.namespaceRescope || urlNamespaces.length !== 1)
      ) {
        const scopedNamespaces = namespaceScope.actives ?? [];
        debugNamespaceLog("app:url-sync-cache-scope-preserve", {
          scopedNamespaces,
        });
    ...
    return {
      ...
      nodes: filteredTopology.nodes.map((node) => {
        ...
        return {
          ...
          data: {
            ...node.data,
            auditDanger: counts.danger,
            auditWarning: counts.warning,
            auditMessages: counts.messages,
          },
        };
      }),
    };
        ...
        <div className="relative flex flex-col flex-1 min-w-0 h-full">
          ...
          {!chromeless && (
            ...
            (its center bar is absolutely positioned). */}
              <div
                className={`flex items-center gap-4 shrink-0 ${showNavRail ? "w-[492px]" : ""}`}
              >
              ...
              shows it in the header (host may override via brandSlot). */}
                ...
                <div
                  className={`flex items-center gap-2 min-w-0 ${showNavRail ? "flex-1" : ""}`}
                >
                  ...
                  <div className="ml-1 flex items-center gap-1.5 shrink-0">
                    ...
                    {showHeaderReconnect && (
                      ...
                      >
                        ...
                        >
                          <RefreshCw
                            className={`w-3 h-3 ${headerReconnectPending ? "animate-spin" : ""}`}
                          />
                        </button>
                      </Tooltip>
                    )}
                ...
                <div className="@min-[920px]:absolute @min-[920px]:left-1/2 @min-[920px]:-translate-x-1/2 flex items-center gap-0.5 bg-theme-elevated/50 rounded-full p-1 ml-2 @min-[920px]:ml-0">
                  {(
                      ...
                      >
                        <button
                          ...
                          className={`flex items-center gap-1 px-2 py-1 text-[13px] rounded-full transition-colors ${
                            mainView === view ||
                            (mainView === "helmCompare" && view === "helm")
                    ...
                    other left-section chrome changes appreciably. */}
                          <span className="hidden @min-[1264px]:inline">
                            {label}
                          </span>
                        </button>
                      </Tooltip>
          ...
          cover already blocks pointer events). */}
            {contentReady && (
              <div className="flex-1 flex overflow-hidden" inert={expandedView}>
                <ErrorBoundary>
                  {/* Home dashboard */}
                  {mainView === "home" && (
                    <HomeView
                      ...
                      topology={topology}
                      fallbackClusterLoadState={
                        showHomeClusterLoadFallback
                          ? clusterLoadState
                          : undefined
                      }
                  ...
                  {mainView === "helmCompare" && <HelmCompareRoute />}
          ...
          {diagnoseOpen && (
            <DiagnoseSurface topInset={chromeless ? 0 : APP_HEADER_HEIGHT} />
          )}
