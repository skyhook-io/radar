...
|---------|-------|-------------|
| Secrets | `rbac.secrets: true` | View secrets in the resource browser and enable the Helm view (Helm stores releases in Secrets). The grant is cluster-wide `get/list/watch` on all Secrets, whose values Radar shows in plaintext; without authentication, anyone who can reach Radar can reveal them. Prefer auth (`auth.mode` other than `none`) or cloud mode, which grant it automatically and re-check every Secret read against the requesting user's own RBAC. Also granted by `rbac.helm`. `rbac.secrets: false` does not remove it in those modes. |
...
| Helm Write | `rbac.helm: true` | Install/upgrade/rollback/uninstall Helm releases. Under auth or cloud-mode, also emits a split helm add-on ClusterRole — `radar-helm` (member-safe: CRDs, storage, namespaces) and `radar-helm-admin` (owner-only: RBAC, webhooks, ApiServices). Not needed to view releases, which is `rbac.secrets` |
