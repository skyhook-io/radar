...
func BuildThrottleQuery(step time.Duration, sel PodSelection, scope WorkloadMetricsScope) (string, error) {
	...
	return buildThrottleQuery(step, sel, matchers, nil)
}

func BuildIdentityThrottleQuery(step time.Duration, sel PodSelection, pods []WorkloadPodIdentity) (string, error) {
	branches, err := buildWorkloadIdentityFilter(pods, "")
	if err != nil {
		return "", err
	}
	return buildThrottleQuery(step, sel, "", branches)
}

func buildThrottleQuery(step time.Duration, sel PodSelection, matchers string, branches *workloadIdentityFilter) (string, error) {
	if sel.IsEmpty() {
	...
	rate := func(metric string) string {
		...
		return "sum by (pod) (max by (pod,container) (" + workloadMetricExpression(metric, matchers, target, branches, WorkloadRateWindow(step).String()) + "))"
	}
...
func BuildWorkloadResourceQuery(step time.Duration, sel PodSelection, scope WorkloadMetricsScope, category MetricCategory) (string, error) {
	...
	return buildWorkloadResourceQuery(step, sel, matchers, category, nil)
}

func BuildIdentityWorkloadResourceQuery(step time.Duration, sel PodSelection, pods []WorkloadPodIdentity, category MetricCategory) (string, error) {
	branches, err := buildWorkloadIdentityFilter(pods, "")
	if err != nil {
		return "", err
	}
	return buildWorkloadResourceQuery(step, sel, "", category, branches)
}

func buildWorkloadResourceQuery(step time.Duration, sel PodSelection, matchers string, category MetricCategory, branches *workloadIdentityFilter) (string, error) {
	if sel.IsEmpty() {
		return "", fmt.Errorf("no current pods")
	}
	target := fmt.Sprintf("namespace=%s%s,container!='',container!='POD'", strconv.Quote(sel.Namespace), scopeClause(sel))
	...
	switch category {
	case CategoryCPU:
		inner = workloadMetricExpression("container_cpu_usage_seconds_total", matchers, target, branches, WorkloadRateWindow(step).String())
	case CategoryMemory:
		inner = workloadMetricExpression("container_memory_working_set_bytes", matchers, target, branches, "")
	default:
